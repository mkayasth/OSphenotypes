...
ta_expression <- ta_expression %>%
  mutate("Gene Status" = case_when(
    ta_results$logFC > 1.5 & ta_results$P.Value < 0.05 ~ "Upregulated in TA phenotype",
    ta_results$logFC < -1.5 & ta_results$P.Value < 0.05 ~ "Downregulated in TA phenotype",
...
ggplot(data = ta_expression, aes(x = logFC, y = -log10(P.Value), color = `Gene Status`)) +
  geom_point() + 
  scale_color_manual(values = c("Upregulated in TA phenotype" = "red", "Downregulated in TA phenotype" = "blue")) +
...
ta_gsva <- gsvaParam(geneExpression, gene_ta, kcdf = "Gaussian")
...
combined_annotation <- HeatmapAnnotation(
  "ALT Genes GSVA Score" = anno_barplot(
    ...
    gp = gpar(fill = ifelse(GSVA_long_alt$Color == "Positive", "blue", "red")),
    border = FALSE,
    height = unit(1,"cm"),
    axis_param = list(at = c(-1, 0, 1), labels = c("-1", "0", "1"))
  ),
  spacer = anno_empty(height = unit(2, "mm"), # adding space between the two annotaitons.
                      border = FALSE), 
  
  "TA Genes GSVA Score" = anno_barplot(
    GSVA_long_ta$GSVA_Score, 
    gp = gpar(fill = ifelse(GSVA_long_ta$Color == "Positive", "blue", "red")),  # Note: corrected from alt to ta
    border = FALSE,
    height = unit(1,"cm"),
    axis_param = list(at = c(-1, 0, 1), labels = c("-1", "0", "1"))
  ),
...
h1 <- Heatmap(
  ...
  column_labels = colnames(geneExpression_scaled_ordered),
  ...
  row_title_gp = gpar(fontsize = 10, col = "black", fontface = "bold"),
